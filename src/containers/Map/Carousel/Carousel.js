...
const carousel = ({allPhotos, restaurantName}) => {
    ...
    const photosCarousel = allPhotos.map((photo, index) => {
        if (index===0) {
            // we need to active the first photo
            return (
                <div key={photo.photo_reference} className="carousel-item active">
                    <img className='d-block' style={{width: 300, height: 250}} src={`https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${photo.photo_reference}&key=${TEMP_API_KEY}`} alt={`${restaurantName}`}/>
                </div>
            );
        }
